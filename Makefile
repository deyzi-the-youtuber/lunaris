...
grub:
	@tools/diskedit.sh mbr.img -m
	@sudo cp $(OBJOUT) /mnt/boot/
	@sudo cp grub.cfg /mnt/boot/grub/
	@tools/diskedit.sh mbr.img -u
	@qemu-img convert -f raw -O vpc mbr.img disk.vhd

qemu:
	@qemu-system-x86_64 -m 2G -drive format=raw,file=mbr.img -serial stdio -cpu core2duo -vga std

debugger:
	@qemu-system-x86_64 -kernel $(OBJOUT) -m 1G -d int -drive format=raw,file=mbr.img -serial stdio -cpu core2duo -vga std
